...
function [lower_bounds,upper_bounds,time]=dasc(T,sampleXsi,M,n,tol,probabilities,lambda,x0,talpha,nb_iter_max)
...
while End_Algo
    ...
    for t=1:T
        ...
        if (t==1)
            ...
            model.Q=sparse([0.5*[zeros(1,n+1);[zeros(n,1),(B+lambda*eye(n))]]]);
        elseif (t==T)
            model.obj=C*trial_states{1,t-1}+sampleXsi{1,t}(Index,n+1:2*n)';
            model.Q=sparse(0.5*(B+lambda*eye(n)));
        else
        ...
        if (t==1)
            thiscost=x(1)+0.5*x0'*A*x0+x0'*C'*trial_states{1,t}+0.5*trial_states{1,t}'*(B+lambda*eye(n))*trial_states{1,t}+sampleXsi{1,t}(Index,1:n)*x0+sampleXsi{1,t}(Index,n+1:2*n)*x(2:n+1);
        elseif (t==T)
            thiscost=0.5*trial_states{1,t-1}'*A*trial_states{1,t-1}+trial_states{1,t}'*C*trial_states{1,t-1}+0.5*trial_states{1,t}'*(B+lambda*eye(n))*trial_states{1,t}+sampleXsi{1,t}(Index,1:n)*trial_states{1,t-1}+sampleXsi{1,t}(Index,n+1:2*n)*x;
        else
            thiscost=x(1)+0.5*trial_states{1,t-1}'*A*trial_states{1,t-1}+trial_states{1,t}'*C*trial_states{1,t-1}+0.5*trial_states{1,t}'*(B+lambda*eye(n))*trial_states{1,t}+sampleXsi{1,t}(Index,1:n)*trial_states{1,t-1}+sampleXsi{1,t}(Index,n+1:2*n)*x(2:n+1);
        end
    ...
    for t=T:-1:2
        if (t==T)
            ...
            for j=1:M
                ...
                results = gurobi(model,params);
                x=results.x;
                objective=0.5*trial_states{1,t-1}'*A*trial_states{1,t-1}+x'*C*trial_states{1,t-1}+0.5*x'*(B+lambda*eye(n))*x+sampleXsi{1,t}(Index,1:n)*trial_states{1,t-1}+sampleXsi{1,t}(Index,n+1:2*n)*x;
                delta=A*trial_states{1,t-1}+C'*x+sampleXsi{1,t}(j,n+1:2*n)';
                objective=objective-delta'*trial_states{1,t-1}+0.5*lambda*trial_states{1,t-1}'*trial_states{1,t-1};
                slope=slope+probabilities{1,t}(j)*(delta-lambda*trial_states{1,t-1});
            ...
            for j=1:M
                ...
                x=results.x;
                objective=x(1)+0.5*trial_states{1,t-1}'*A*trial_states{1,t-1}+x(2:n+1)'*C*trial_states{1,t-1}+0.5*x(2:n+1)'*(B+lambda*eye(n))*x(2:n+1)+sampleXsi{1,t}(Index,1:n)*trial_states{1,t-1}+sampleXsi{1,t}(Index,n+1:2*n)*x(2:n+1);
                delta=A*trial_states{1,t-1}+C'*x(2:n+1)+sampleXsi{1,t}(j,n+1:2*n)';
                objective=objective-delta'*trial_states{1,t-1}+0.5*lambda*trial_states{1,t-1}'*trial_states{1,t-1};
                slope=slope+probabilities{1,t-1}(j)*(delta-lambda*trial_states{1,t-1});
